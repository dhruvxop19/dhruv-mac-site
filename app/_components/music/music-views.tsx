...
const CURATED_SONGS = frequentlyPlayedIds
  .map((id) => songById[id])
  .filter(Boolean) as Song[];
const CURATED_SONG_IDS = CURATED_SONGS.map((song) => song.id);
const CURATED_ALBUM_IDS = new Set(CURATED_SONGS.map((song) => song.albumId));
const CURATED_ARTIST_IDS = new Set(
  CURATED_SONGS
    .map((song) => albumById[song.albumId]?.artistId)
    .filter(Boolean) as string[],
);
const CURATED_ALBUMS = albums.filter((album) => CURATED_ALBUM_IDS.has(album.id));
const CURATED_ARTISTS = artists.filter((artist) => CURATED_ARTIST_IDS.has(artist.id));
...
export function HomeView({
  ...
  return (
    <div className={styles.contentScroll} data-window-drag-ignore>
      ...
      {/* Featured picks */}
      <p className={styles.pageSubTitle}>Top Picks for Dhruv</p>
...
export function AlbumDetailView({
  ...
  const albumSongs = album.songIds
    .map((id) => songById[id])
    .filter((song): song is Song => Boolean(song) && CURATED_SONG_IDS.includes(song.id));
        ...
        <div className={styles.albumHeroInfo}>
          ...
          <div className={styles.albumHeroActions}>
            <button
              ...
              data-window-drag-ignore
              onClick={() =>
                player.play(albumSongs[0].id, albumSongs.map((song) => song.id))
              }
            >
              <IconPlay />
      ...
      <div className={styles.albumTrackList}>
        {albumSongs.map((song, i) => (
          <SongRow
            ...
            isCurrent={player.currentSong?.id === song.id}
            onPlay={player.play}
            contextIds={albumSongs.map((song) => song.id)}
            showArtwork={false}
          />
        ))}
      </div>
...
export function ArtistDetailView({
  ...
  const artistAlbums = artist.albumIds
    .map((id) => albumById[id])
    .filter((album): album is Album => Boolean(album) && CURATED_ALBUM_IDS.has(album.id));
  ...
  const artistSongs = CURATED_SONGS.filter((s) => {
